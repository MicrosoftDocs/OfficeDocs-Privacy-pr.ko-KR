...
**데이터 검토** 단계는 공동 작업자가 **데이터 수집** 탭의 콘텐츠 항목을 검사하는 경우입니다. 모든 이해 관계자의 콘텐츠 검토를 용이하게 하기 위해 Teams 채널이 자동으로 설정됩니다. 자세한 내용은 아래 [의 데이터 검토에 대한 공동 작업을](#collaboration-for-data-review) 참조하세요. 데이터 검토 단계의 필수 작업은 아래에 설명되어 있습니다.

#### <a name="import-additional-files"></a>추가 파일 가져오기

데이터 검토 요청에 추가 콘텐츠 항목을 가져올 수 있습니다. 예를 들어 조직의 Microsoft 365 환경 외부에 저장된 파일 또는 관련성이 있지만 검색 범위가 지정되지 않은 기타 항목이 있습니다. 다른 항목과 함께 검토 및 작업할 개별 요청의 **데이터 수집** 탭으로 파일을 가져올 수 있습니다. 가져온 파일은 검색에서 검색된 다른 콘텐츠 항목과 함께 동일한 Azure Blob Storage 컨테이너에 추가됩니다.

아래 단계에 따라 파일을 가져옵니다.

1. 요청 세부 정보 페이지에서 페이지 맨 위에 있는 명령 모음에서 **파일 가져오기** 를 선택합니다.

2.  **파일 가져오기** 상자가 화면에 표시됩니다. **파일 선택을 선택하고 파일** 탐색기 보기에서 가져올 파일을 하나 이상 선택합니다.

3. 선택한 파일이 나열된 **파일 가져오기** 상자로 돌아갑니다. **파일 선택을** 다시 선택하여 목록에 항목을 더 추가할 수 있습니다. 파일을 제거하려면 **지우** 기를 선택하여 모든 파일을 제거하고 파일을 다시 선택합니다.

4. 원하는 모든 파일이 **파일 가져오기** 상자에 나열되면 **가져오기** 를 선택합니다. 업로드하지 않고 종료하려면 **가져오기 취소** 를 선택합니다.

가져오기가 시작되면 요청의 **데이터 수집** 탭으로 돌아갑니다. 탭 위의 메시지는 업로드가 진행 중임을 나타냅니다. 업로드에 문제가 있는 경우 메시지가 알려주고 다시 시도할 수 있는 옵션을 제공합니다.

가져오기가 완료되면 **수집된 데이터** 탭 위에 확인 메시지가 표시됩니다.

**파일 가져오기에 대한 자세한 내용:**

- 개별 파일 크기는 4MB를 초과할 수 없습니다. 파일이 너무 커서 업로드할 수 없는 경우 **파일 가져오기** 상자에 경고 메시지가 표시됩니다.

- 가져온 파일을 **데이터 수집** 탭에서 사용할 수 있는 데 최대 20분이 걸릴 수 있습니다.

- 한 사용자에 대해 가져오기가 이미 진행 중인 경우 이전 업로드 프로세스가 완료될 때까지 동일한 사용자가 추가 파일을 업로드할 수 없습니다. 여러 사용자가 동일한 요청에 동시에 파일을 업로드할 수 있습니다. 그러나 진행 중인 업로드가 많을수록 완료하는 데 시간이 오래 걸립니다. 요청의 상태 메시지는 업로드가 완료되고 파일을 검토할 준비가 되면 알려줍니다.

#### <a name="mark-items-as-include-or-exclude-and-add-notes"></a>항목을 포함 또는 제외로 표시하고 노트 추가

요청의 **데이터 수집** 탭에 나열된 항목을 검토합니다.  항목이 최종 보고서의 일부로 데이터 주체로 다시 포함되도록 결정한 경우 항목 목록의 맨 위에 있는 명령 모음에서 **[포함** ]을 선택합니다. 항목 목록 오른쪽의 콘텐츠 검토 영역에서 파란색 **포함** 단추를 선택할 수도 있습니다. **[포함**]을 선택하면 슬라이드 노트를 추가하는 옵션이 있는 플라이아웃 창이 나타납니다. 완료되면 **제출** 을 선택하여 항목의 검토 상태를 **포함** 으로 저장합니다.

항목이 요청의 일부로 속하지 않는 경우 명령 모음에서 **제외** 또는 콘텐츠 검토 영역의 **제외** 단추를 선택할 수 있습니다. 내부 레코드에는 항목을 명시적으로 **제외** 로 표시해야 하는 경우가 많습니다.

기본적으로 **포함** 으로 표시한 항목만 [데이터 주체에 대해 생성된 최종 보고서에 포함됩니다](subject-rights-requests-reports.md).

> [!NOTE]
> 항목을 **제외** 로 표시하는 경우 주체 권한 요청과 관련이 없는 이유에 대한 근거로 메모를 추가해야 합니다. 참고는 내부용이며 최종 보고서에 포함되지 않습니다.

콘텐츠가 검색 쿼리에 대해 가양성으로 표시되는 경우 **일치하지 않음을** 선택하고 플라이아웃 창에서 **확인을** 선택합니다. 이 작업은 검색에서 검색되지 않아야 하는 항목으로 플래그를 지정합니다.
...
주체 권한 요청의 오른쪽 위 모서리에 있는 설정 Priva 이동하여 주체 권한 요청에 대한 **Teams** 채널을 만드는 기본 동작을 변경할 수 있습니다. **Teams 협업** 을 선택한 다음 페이지에서 확인란의 선택을 취소하여 모든 주체 권한 요청에 대한 Teams 기능을 해제합니다.
